...
void GLRenderDevice::SetVertexFormat(uint8_t format, uintptr_t offset)
{
...
#endif
    {
        ...
        if ((format & VFTex2) > 0)
        {
            V(glVertexAttribPointer(Attr::Tex2, 4, GL_UNSIGNED_SHORT, GL_TRUE, stride, (void*)offset));
            offset += 8;
        }
...
namespace
{
    ...
    namespace GLES2
    {
        #include "ShadersGLES2.h"
    }
    namespace GLES3
    {
        #include "ShadersGLES3.h"
    }
#endif
}
...
void GLRenderDevice::CreatePrograms()
{
    ...
    const uint8_t* source = isGL31 ? GL31::Shaders : GL21::Shaders;
    const uint32_t size = isGL31 ? sizeof(GL31::Shaders) : sizeof(GL21::Shaders);    
    #define SHADER(type, n) CompileShader(type, \
        isGL31 ? shaders + GL31::n##_Start : shaders + GL21::n##_Start,  \
        isGL31 ? GL31::n##_Size : GL21::n##_Size)
#else
    bool isGLES3 = mGLVersion > NS_GL_VER(2,0);
    const uint8_t* source = isGLES3 ? GLES3::Shaders : GLES2::Shaders;
    const uint32_t size = isGLES3 ? sizeof(GLES3::Shaders) : sizeof(GLES2::Shaders);
    #define SHADER(type, n) CompileShader(type, \
        isGLES3 ? shaders + GLES3::n##_Start : shaders + GLES2::n##_Start,  \
        isGLES3 ? GLES3::n##_Size : GLES2::n##_Size)
#endif

    char* shaders = (char*)Alloc(FastLZ::DecompressBufferSize(source));
    FastLZ::Decompress(source, size, shaders);

    #define VSHADER(n) SHADER(GL_VERTEX_SHADER, n)
    #define FSHADER(n) SHADER(GL_FRAGMENT_SHADER, n)
    ...
    GLuint vShaders[] =
    {
        ...
        VSHADER(PosColorTex1Tex2_VS),
        VSHADER(PosTex0Tex1Tex2_VS),
        VSHADER(PosColorTex1_SDF_VS),
        VSHADER(PosTex0Tex1_SDF_VS),
    };
    ...
    GLuint fShaders[] =
    {
        ...
        FSHADER(SDFSolid_FS),
        FSHADER(SDFLinear_FS),
        FSHADER(SDFRadial_FS),
        FSHADER(SDFPattern_FS),

        FSHADER(ImageOpacitySolid_FS),
        FSHADER(ImageOpacityLinear_FS),
        FSHADER(ImageOpacityRadial_FS),
        FSHADER(ImageOpacityPattern_FS),

        FSHADER(ImageShadow35V_FS),
        FSHADER(ImageShadow63V_FS),
        FSHADER(ImageShadow127V_FS),

        FSHADER(ImageShadow35HSolid_FS),
        FSHADER(ImageShadow35HLinear_FS),
        FSHADER(ImageShadow35HRadial_FS),
        FSHADER(ImageShadow35HPattern_FS),

        FSHADER(ImageShadow63HSolid_FS),
        FSHADER(ImageShadow63HLinear_FS),
        FSHADER(ImageShadow63HRadial_FS),
        FSHADER(ImageShadow63HPattern_FS),

        FSHADER(ImageShadow127HSolid_FS),
        FSHADER(ImageShadow127HLinear_FS),
        FSHADER(ImageShadow127HRadial_FS),
        FSHADER(ImageShadow127HPattern_FS),

        FSHADER(ImageBlur35V_FS),
        FSHADER(ImageBlur63V_FS),
        FSHADER(ImageBlur127V_FS),

        FSHADER(ImageBlur35HSolid_FS),
        FSHADER(ImageBlur35HLinear_FS),
        FSHADER(ImageBlur35HRadial_FS),
        FSHADER(ImageBlur35HPattern_FS),

        FSHADER(ImageBlur63HSolid_FS),
        FSHADER(ImageBlur63HLinear_FS),
        FSHADER(ImageBlur63HRadial_FS),
        FSHADER(ImageBlur63HPattern_FS),

        FSHADER(ImageBlur127HSolid_FS),
        FSHADER(ImageBlur127HLinear_FS),
        FSHADER(ImageBlur127HRadial_FS),
        FSHADER(ImageBlur127HPattern_FS)
    };
    ...
    const char* attribs[] =
    {
        "attr_pos", "attr_color", "attr_tex0", "attr_tex1", "attr_tex2", "attr_coverage", 0
    };

    static_assert(NS_COUNTOF(mPrograms) == NS_COUNTOF(programs), "");

    // Do a first pass without checking errors to allow link in parallel on background threads
    ...
    for (uint32_t i = 0; i < NS_COUNTOF(programs); i++)
    {
        ...
        {
            NS_ASSERT(p.vShaderIdx < (int)NS_COUNTOF(vShaders));
            NS_ASSERT(p.fShaderIdx < (int)NS_COUNTOF(fShaders));
    ...
    for (uint32_t i = 0; i < NS_COUNTOF(programs); i++)
    {
        ...
        if (po != 0)
        {
            ...
            mPrograms[i].textureSizeLocation = glGetUniformLocation(po, "textureDimensions");
            mPrograms[i].textureSizeHash = 0;
            ...
            mPrograms[i].effectParamsLocation = glGetUniformLocation(po, "effectParams");
            mPrograms[i].effectParamsHash = 0;
        }
    }
...
void GLRenderDevice::CreateVertexFormats()
{
    ...
    if (IsSupported(Extension::ARB_vertex_attrib_binding))
    {
        ...
        for (uint32_t i = 0; i < NS_COUNTOF(mVertexFormats); i++)
        {
            ...
            if ((i & VFTex2) > 0)
            {
                V(glEnableVertexAttribArray(Attr::Tex2));
                V(glVertexAttribBinding(Attr::Tex2, 0));
                V(glVertexAttribFormat(Attr::Tex2, 4, GL_UNSIGNED_SHORT, GL_TRUE, offset));
                offset += 8;
            }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
GLuint GLRenderDevice::CompileShader(GLenum type, const char* source, uint32_t len) const
{
    GLuint shader = glCreateShader(type);
    V(glShaderSource(shader, 1, &source, (GLint*)&len));
...
GLuint GLRenderDevice::LinkProgram(GLuint vso, GLuint pso, const char** attribs) const
{
    ...
    if (attribs != 0)
    {
        ...
        {
            V(glBindAttribLocation(po, i, attribs[i]));
        }
    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GLRenderDevice::ValidateProgram(GLuint po) const
{
    GLint status;
    ...
    if (status == 0)
    {
        ...
        {
            char error[512];
            V(glGetProgramInfoLog(po, 512, 0, error));
            V(glDeleteProgram(po));
            NS_FATAL("Link failed\n%s", error);
        }
    }
}
...
{
    NS_ASSERT(batch.shader.v < NS_COUNTOF(mPrograms));
    ProgramInfo& program = mPrograms[batch.shader.v];

    // Use program
    NS_ASSERT(program.object != 0);
    UseProgram(program.object);

    // Upload constants
    if (program.projectionMtxHash != batch.projMtxHash)
    {
        V(glUniformMatrix4fv(program.projectionMtxLocation, 1, GL_FALSE, *batch.projMtx));
        program.projectionMtxHash = batch.projMtxHash;
    }

    if (program.textureSizeLocation != -1)
    {
        GLTexture* texture = (GLTexture*)(batch.glyphs ? batch.glyphs : batch.image);
        NS_ASSERT(texture != 0);
        ...
        if (hash != program.textureSizeHash)
        {
            GLfloat p[] = { (float)width, (float)height, 1.0f / width, 1.0f / height };
            V(glUniform4f(program.textureSizeLocation, p[0], p[1], p[2], p[3]));
            program.textureSizeHash = hash;
        }
    }

    if (program.opacityLocation != -1 && program.opacityHash != batch.opacityHash)
    {
        NS_ASSERT(batch.opacity != 0);
        V(glUniform1f(program.opacityLocation, *batch.opacity));
        program.opacityHash = batch.opacityHash;
    }

    if (program.radialGradLocation != -1 && program.radialGradHash != batch.radialGradHash)
    {
        NS_ASSERT(batch.radialGrad != 0);
        V(glUniform4fv(program.radialGradLocation, 2, *batch.radialGrad));
        program.radialGradHash = batch.radialGradHash;
    }

    if (program.rgbaLocation != -1 && program.rgbaHash != batch.rgbaHash)
    {
        ...
        V(glUniform4f(program.rgbaLocation, v0, v1, v2, v3));
        program.rgbaHash = batch.rgbaHash;
    }

    if (program.effectParamsLocation != -1 && program.effectParamsHash != batch.effectParamsHash)
    {
        NS_ASSERT(batch.effectParams != 0);
        V(glUniform1fv(program.effectParamsLocation, batch.effectParamsSize, batch.effectParams));
        program.effectParamsHash = batch.effectParamsHash;
    }
}
