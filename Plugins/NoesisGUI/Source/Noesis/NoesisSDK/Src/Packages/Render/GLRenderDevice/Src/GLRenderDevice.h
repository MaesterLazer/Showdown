...
private:
    ...
    GLuint CompileShader(GLenum type, const char* source, uint32_t len) const;
    GLuint LinkProgram(GLuint vso, GLuint pso, const char** binds) const;
    void ValidateProgram(GLuint po) const;
...
private:
    ...
    VertexFormat mVertexFormats[32];
    ...
    struct ProgramInfo
    {
        ...
        GLint rgbaLocation;
        GLint effectParamsLocation;

        uint32_t projectionMtxHash;
        uint32_t textureSizeHash;
