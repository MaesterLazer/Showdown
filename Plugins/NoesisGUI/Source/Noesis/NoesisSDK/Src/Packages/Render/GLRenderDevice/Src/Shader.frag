...
uniform sampler2D shadow;
    ...
    #define GAUSSIAN_NUM_SAMPLES 9
    #if defined(PRECISION_QUALIFIERS) && !defined(CORE_PROFILE)
        ...
        #define DECLARE_SAMPLES \
            float w[GAUSSIAN_NUM_SAMPLES]; \
            float o[GAUSSIAN_NUM_SAMPLES]; \
            SAMPLE(0, 0.10855, 0.66293) \
            SAMPLE(1, 0.13135, 2.47904) \
            SAMPLE(2, 0.10406, 4.46232) \
            SAMPLE(3, 0.07216, 6.44568) \
            SAMPLE(4, 0.04380, 8.42917) \
            SAMPLE(5, 0.02328, 10.41281) \
            SAMPLE(6, 0.01083, 12.39664) \
            SAMPLE(7, 0.00441, 14.38070) \
            SAMPLE(8, 0.00157, 16.36501)
    #else
        #define DECLARE_SAMPLES \
    ...
    #define GAUSSIAN_NUM_SAMPLES 16
        ...
        #define DECLARE_SAMPLES \
            float w[GAUSSIAN_NUM_SAMPLES]; \
            float o[GAUSSIAN_NUM_SAMPLES]; \
            SAMPLE(0, 0.05991, 0.66555) \
            SAMPLE(1, 0.07758, 2.49371) \
            SAMPLE(2, 0.07232, 4.48868) \
            SAMPLE(3, 0.06476, 6.48366) \
            SAMPLE(4, 0.05571, 8.47864) \
            SAMPLE(5, 0.04604, 10.47362) \
            SAMPLE(6, 0.03655, 12.46860) \
            SAMPLE(7, 0.02788, 14.46360) \
            SAMPLE(8, 0.02042, 16.45860) \
            SAMPLE(9, 0.01438, 18.45361) \
            SAMPLE(10, 0.00972, 20.44863) \
            SAMPLE(11, 0.00631, 22.44365) \
            SAMPLE(12, 0.00394, 24.43869) \
            SAMPLE(13, 0.00236, 26.43375) \
            SAMPLE(14, 0.00136, 28.42881) \
            SAMPLE(15, 0.00075, 30.42389)
    #else
        #define DECLARE_SAMPLES \
            const float w[GAUSSIAN_NUM_SAMPLES] = float[] \
            ( \
    ...
    #define GAUSSIAN_NUM_SAMPLES 32
        ...
        #define DECLARE_SAMPLES \
            float w[GAUSSIAN_NUM_SAMPLES]; \
            float o[GAUSSIAN_NUM_SAMPLES]; \
            SAMPLE(0, 0.02954, 0.66640) \
            SAMPLE(1, 0.03910, 2.49848) \
            SAMPLE(2, 0.03844, 4.49726) \
            SAMPLE(3, 0.03743, 6.49605) \
            SAMPLE(4, 0.03609, 8.49483) \
            SAMPLE(5, 0.03446, 10.49362) \
            SAMPLE(6, 0.03259, 12.49240) \
            SAMPLE(7, 0.03052, 14.49119) \
            SAMPLE(8, 0.02830, 16.48997) \
            SAMPLE(9, 0.02600, 18.48876) \
            SAMPLE(10, 0.02365, 20.48754) \
            SAMPLE(11, 0.02130, 22.48633) \
            SAMPLE(12, 0.01900, 24.48511) \
            SAMPLE(13, 0.01679, 26.48390) \
            SAMPLE(14, 0.01469, 28.48268) \
            SAMPLE(15, 0.01272, 30.48147) \
            SAMPLE(16, 0.01092, 32.48026) \
            SAMPLE(17, 0.00928, 34.47904) \
            SAMPLE(18, 0.00781, 36.47783) \
            SAMPLE(19, 0.00651, 38.47662) \
            SAMPLE(20, 0.00537, 40.47540) \
            SAMPLE(21, 0.00439, 42.47419) \
            SAMPLE(22, 0.00355, 44.47298) \
            SAMPLE(23, 0.00285, 46.47176) \
            SAMPLE(24, 0.00226, 48.47055) \
            SAMPLE(25, 0.00178, 50.46934) \
            SAMPLE(26, 0.00138, 52.46813) \
            SAMPLE(27, 0.00107, 54.46692) \
            SAMPLE(28, 0.00081, 56.46571) \
            SAMPLE(29, 0.00062, 58.46450) \
            SAMPLE(30, 0.00046, 60.46329) \
            SAMPLE(31, 0.00034, 62.46208)
    #else
        #define DECLARE_SAMPLES \
            const float w[GAUSSIAN_NUM_SAMPLES] = float[] \
            ( \
...
void main()
{
    ...
    /////////////////////////////////////////////////////
        ...
        float alpha = 0.0;
        vec2 dir = vec2(0, BLUR_SIZE * textureDimensions.w);
        ...
        for (int i = 0; i < GAUSSIAN_NUM_SAMPLES; i++)
        {
            ...
            alpha += w[i] * (texture2D(image, clamp(uv1 + uvOffset, uv2.xy, uv2.zw)).a + texture2D(image, clamp(uv1 - uvOffset, uv2.xy, uv2.zw)).a);
        }
        ...
        float alpha = 0.0;
        vec2 dir = vec2(BLUR_SIZE * textureDimensions.z, 0);
        vec2 offset = vec2(SHADOW_OFFSETX * textureDimensions.z, SHADOW_OFFSETY * textureDimensions.w);
        ...
        FRAG_COLOR = (img + (1.0 - img.a) * (SHADOW_COLOR * alpha)) * (opacity_ * paint.a);
        ...
        {
            vec2 uvOffset = o[i] * dir;
            color += w[i] * (texture2D(image, clamp(uv1 + uvOffset, uv2.xy, uv2.zw)) + texture2D(image, clamp(uv1 - uvOffset, uv2.xy, uv2.zw)));
        }
        ...
        FRAG_COLOR = color * (opacity_ * paint.a);
        ...
        FRAG_COLOR = (alpha * opacity_) * paint;
