...
AppKitDisplay::AppKitDisplay(): mClosed(false)
{
    @autoreleasepool
    {
        ...
        CGFloat width = 74 * screenWidth / 100;
        CGFloat height = width * screenHeight / screenWidth;
        NSRect frame = NSMakeRect(0, 0, width, height);
        NSUInteger styleMask = NSWindowStyleMaskTitled | NSWindowStyleMaskClosable |
            NSWindowStyleMaskMiniaturizable | NSWindowStyleMaskResizable;

        mWindow = [[WindowDelegate alloc] initWithContentRect:frame styleMask:styleMask
            backing: NSBackingStoreBuffered defer:FALSE];

        [mWindow setDisplay:this];
        ...
        mTextInputClient = [[TextInputClient alloc] initWithDisplay:this];
        [[mWindow contentView] addSubview: mTextInputClient];
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void AppKitDisplay::EnterMessageLoop(bool runInBackground)
{
    @autoreleasepool
    {
        ...
        NSDate* wait = [NSDate distantFuture];
        NSDate* peek = [NSDate distantPast];
        ...
        for (;;)
        {
            @autoreleasepool
            {
                for (;;)
                {
                    ...
                    bool sleep = !inspector && !mWindow.isKeyWindow && !runInBackground && !firstFrame;
                    firstFrame = false;

                    NSEvent* event = [NSApp nextEventMatchingMask:NSAnyEventMask
                        untilDate: sleep ? wait : peek inMode:NSDefaultRunLoopMode dequeue:YES];

                    if (event == nil || mClosed)
                    {
                        break;
                    }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void AppKitDisplay::OpenSoftwareKeyboard(Noesis::UIElement* focused)
{
    TextBox* textBox = DynamicCast<TextBox*>(focused);

    if (textBox != nullptr)
    {
        [mTextInputClient setControl:textBox];
        [mWindow makeFirstResponder: mTextInputClient];
    }
}

////////////////////////////////////////////////////////////////////////////////////////////////////
void AppKitDisplay::CloseSoftwareKeyboard()
{
    [mWindow makeFirstResponder: mWindow];
}

////////////////////////////////////////////////////////////////////////////////////////////////////
void AppKitDisplay::SetCursor(Noesis::Cursor cursor_)
{
    NSView* view = mWindow.contentView;
    NSCursor* cursor = mCursors[cursor_] != nullptr ? mCursors[cursor_] : [NSCursor arrowCursor];
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void AppKitDisplay::OpenUrl(const char* url_)
{
    NSWorkspace* workspace = [NSWorkspace sharedWorkspace];

    NSString* str = [NSString stringWithUTF8String:url_];
    str = [str stringByReplacingOccurrencesOfString:@".exe" withString:@".app"];

    NSBundle* mainBundle = [NSBundle mainBundle];
    NSString* bundleFolder = [[mainBundle bundlePath] stringByDeletingLastPathComponent];

    if ([str hasSuffix:@".app"])
    {
        NSString* app = [bundleFolder stringByAppendingString: @"/"];
        app = [app stringByAppendingString: str];
    ...
    else
    {
        ...
        if (![workspace openURL:url options:NSWorkspaceLaunchDefault configuration:dict error:0])
        {
            NSURL* base = [NSURL fileURLWithPath:bundleFolder];
            url = [NSURL URLWithString:str relativeToURL:base];
            [workspace openURL:url options:NSWorkspaceLaunchDefault configuration:dict error:0];
        }
...
////////////////////////////////////////////////////////////////////////////////////////////////////
float AppKitDisplay::GetScale() const
{
    return (float)mWindow.backingScaleFactor;
}

////////////////////////////////////////////////////////////////////////////////////////////////////
void AppKitDisplay::AdjustWindowSize(uint32_t& width, uint32_t& height) const
{
    NSRect rect = [mWindow frameRectForContentRect:NSMakeRect(0, 0, width, height)];
...
////////////////////////////////////////////////////////////////////////////////////////////////////
void AppKitDisplay::LoadCursors()
{
    memset(mCursors, 0, sizeof(mCursors));

    mCursors[Cursor_Arrow] = [NSCursor arrowCursor];
    mCursors[Cursor_IBeam] = [NSCursor IBeamCursor];
    mCursors[Cursor_Cross] = [NSCursor crosshairCursor];
    mCursors[Cursor_Hand] = [NSCursor pointingHandCursor];
    mCursors[Cursor_SizeNS] = [NSCursor resizeUpDownCursor];
    mCursors[Cursor_SizeWE] = [NSCursor resizeLeftRightCursor];
    mCursors[Cursor_No] = [NSCursor operationNotAllowedCursor];
}
