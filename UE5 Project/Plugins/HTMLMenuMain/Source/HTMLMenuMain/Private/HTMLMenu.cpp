...
void UHTMLMenu::appReactivtedDelegate() {
	...
	UHTMLMenuMainBPLibrary::getHTMLMenuMainBPLibrary()->initWebsocketServer(this);
	//	if (GEngine)
...
void UHTMLMenu::reconnectMobile() {
...
UHTMLMenu* UHTMLMenu::generateHTML_File(FString sourceHTML_File, FString targetHTML_Dir, bool overwriteExistingFile, bool onlyInEditor) {
	//run only in editor
	if (onlyInEditor) {
#if !WITH_EDITOR
		return this;
...
UHTMLMenu* UHTMLMenu::loadHTML_File(FString htmlFile, bool removeLineBreaks) {
	...
	if (isFileRead) {
		...
		LoadString(content, htmlMenuDir);
...
void UHTMLMenu::LoadAllFilesFromDirectory(EHTML_DirectoryType directoryType, EHTML_FileType fileType, FString dirPath, int32& count, TArray<FString>& files, TArray<FString>& filePaths){
...
bool UHTMLMenu::LoadPDF_FileIntoViewer(EHTML_DirectoryType directoryType, FString dirPath){
	if (dirPath.Len() < 1) {
		return false;
	}
...
void UHTMLMenu::getSystemType(EHtmlMenuSystem& system){
	#if PLATFORM_ANDROID
		system = EHtmlMenuSystem::Android;
		return;
	#endif	
	#if PLATFORM_IOS
		system = EHtmlMenuSystem::IOS;
		return;
	#endif	
	#if PLATFORM_WINDOWS
		system = EHtmlMenuSystem::Windows;
		return;
	#endif	
	#if PLATFORM_LINUX
		system = EHtmlMenuSystem::Linux;
		return;
	#endif	
	#if PLATFORM_MAC
		system = EHtmlMenuSystem::Mac;
		return;
	#endif	
}

void UHTMLMenu::stringToBase64String(FString string, FString& base64String) {
	base64String.Empty();
...
void UHTMLMenu::base64StringToString(FString& string, FString base64String) {
	string.Empty();
	TArray<uint8> bytes;
...
void UHTMLMenu::bytesToBase64String(TArray<uint8> bytes, FString& base64String) {
	base64String.Empty();
	base64String = FBase64::Encode(bytes);
}

TArray<uint8> UHTMLMenu::base64StringToBytes(FString base64String, bool& success) {
	TArray<uint8> fileData;
...
void UHTMLMenu::fileToBase64String(EHTML_DirectoryType directoryType, FString filePath, bool& success, FString& base64String, FString& fileName) {
	base64String.Empty();
	fileName.Empty();
	TArray<uint8> fileData;
...
FString UHTMLMenu::getCleanDirectory(EHTML_DirectoryType directoryType, FString filePath) {
	if (directoryType == EHTML_DirectoryType::E_ad) {
...
void UHTMLMenu::ExecuteJavascript(const FString& ScriptText)
{
	...
	if (this->IsValidLowLevel() && HTMLMenu.IsValid())
	{
		/*if (GEngine)
