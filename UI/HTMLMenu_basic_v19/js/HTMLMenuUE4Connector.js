...
var showLog 				= false;/*Redirect console.log to a Textarea. Use the showAlert() function*/
var showAlertIfUE4NotFound	= false;
var keyboardControls		= true;/*Start keyboard key listener*/
var sendMousePositionToUE4	= true;
...
function sendAndClearStack(message){
	...
	setTimeout(function () { 
		...
		if (mobileActionsStack.length > 0){
			sendAndClearStack(mobileActionsStack.shift());
			/*showAlert("send stack");*/
		}
	}, 200);
	
};

/**Redirect console.log to a Textarea*/
function initLoging(){
	if (!showLog)
		return;
	var newConsole = document.getElementById("consoleTextArea");
	if (!newConsole && document.body){
		document.body.innerHTML = '<textarea id="consoleTextArea" style="position:absolute;bottom: 0;height: 200px;width: 90%;"></textarea>'+document.body.innerHTML;
		newConsole = document.getElementById("consoleTextArea");
		var oldLog = console.log;
		console.log = function (message) {
			if (typeof message == 'object'){
				message = message+ " " + JSON.stringify(message);
			}
			newConsole.value = newConsole.value + message+"\r";
